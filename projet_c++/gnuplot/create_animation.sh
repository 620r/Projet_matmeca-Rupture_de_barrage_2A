...
if [ -z "$BASE_NAME" ] || [ -z "$STEP_SKIP" ]; then
    ...
    echo "Usage: ./gnuplot/create_animation.sh NOM_BASE K_PAS_DE_TEMPS"
    echo "Exemple (tous les 10 pas de temps): ./gnuplot/create_animation.sh premiere_sortie 10"
    echo "Exemple (tous les pas de temps): ./gnuplot/create_animation.sh premiere_sortie 1"
    echo "------------------------------------------------------"
    exit 1
fi
...
INPUT_DIR="output"
OUTPUT_DIR="gnuplot/frames"
GIF_OUTPUT="gnuplot/${BASE_NAME}_animation_skip_${STEP_SKIP}.gif" 
SCRIPT_GP="gnuplot/plot_frame.gp"
SEARCH_PATTERN="${BASE_NAME}_iter_*.csv"
...
while read FULL_PATH_IN; do
    ...
    FRAME_OUT="${OUTPUT_DIR}/frame_$(printf "%04d" $FRAME_COUNT).png"
